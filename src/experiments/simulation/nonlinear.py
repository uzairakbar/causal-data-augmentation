...
def run_experiment(args):
    ...
    all_functions = {
        func: {
            name: np.zeros((100, args["n_experiments"])) for name in methods
        } for func in SEM.get_functions()
    }
    all_errors = {
        func: {
            name: np.zeros(args["n_experiments"]) for name in methods
        } for func in SEM.get_functions()
    }
    ...
    for j, (sem, da) in enumerate(zip(all_sems, all_augmenters)):
        ...
        for i in range(args["n_experiments"]):
            print(f"{sem.function_name}: {i}")
        
            X, y = sem(N = args["n_samples"])
            GX, G = da(X)

            X_test, _ = sem(N = args["n_samples"])
            y_test = sem.f(X_test)
    ...
    return all_functions, all_errors


def main():
    parser = argparse.ArgumentParser(description='Nonlinear Simulation')
